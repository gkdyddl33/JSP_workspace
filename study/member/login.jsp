...
<%
   ...
   //클라이언트가 전송한 id,pass파라미터를 받아와 처리해보자!

   //request 객체를 이용하면, 클라이언트의 요청을 처리할 수 있다..
   String id = request.getParameter("id"); 
   //클라이언트가 요청시 전송한 파라미터값을 얻는 메서드
   //매개변수로는 파라미터 변수명을 명시 ,만일 html인경우
   //컴포넌트에 부여된 .name값을 명시할 수 있다.   

   String pass = request.getParameter("pass");
   out.print("<br>");
   out.print("현재 클라이언트의 요청 방식은 "+request.getMethod());
   out.print("<br>");
   out.print("클이언트가 전송한 id는 ?? "+id);
   out.print("<br>");
   out.print("클라이언트가 전송한 password는 ?? "+pass);
